$(document).ready(function () {
  // 모든 path와 ellipse 요소 선택
  $("path, ellipse").each(function (index) {
    // 각 요소의 경로 길이 가져오기
  ...
  $targets.each(function () {
    ...
    // SplitType으로 텍스트를 분할
    let splitClient = new SplitType($target[0], {
      type: "lines, chars",
    });

    // 분할된 문자에 부모 클래스의 스타일 상속 적용
    $target.find(".light").each(function () {
      const lightChars = $(this).find(".char");
      lightChars.css({
        "font-family": "'GmarketSansLight'", // 얇은 글꼴 적용
      });
    });
  ...
  function createLantern() {
    ...
    const randomSize = Math.random() * 0.5 + 1.1;
    const randomSpeed = Math.random() * 5 + 7;
    const randomPosition = Math.random() * 100;

    const lantern = $('<img class="lantern">').attr("src", randomImage);
    lantern.css({
      width: `${randomSize * 80}px`,
      left: `${randomPosition}%`,
      bottom: "0px",
    ...
    lantern.on("animationend", function () {
      $(this).remove();
    });
  }
  ...
  function createSwingingLantern() {
    ...
    const randomSize = Math.random() * 0.7 + 1; // 크기 랜덤화
    ...
    swingingLantern.css({
      width: `${randomSize * 70}px`, // 크기 조정 (원근감을 위해 다양한 크기 설정)
      bottom: `${randomBottomPosition}px`,
    ...
    $.each($(".swinging-lantern"), function (i, d) {
      var rndSec = Math.random() * 2;
      var rndOp = Math.random() / 5 + 0.8;
      $(d).css("animation-delay", rndSec + "s");
      $(d).css("opacity", rndOp);
    });
  }
  ...
  function createBlurredLantern() {
    const randomImage =
      lanternImages[Math.floor(Math.random() * lanternImages.length)];
    const randomSize = Math.random() * 0.4 + 0.6; // 작은 크기 (0.3 ~ 0.6)
    const randomSpeed = Math.random() * 20 + 15; // 천천히 올라가도록 (15초 ~ 35초)
    const randomPosition = Math.random() * 100; // 화면 가로 위치 (0% ~ 100%)
    const randomBottomPosition = Math.random() * 50 + 100; // 화면 중간 위쪽에서 시작 (200px ~ 400px)
